...
import com.snailscoder.core.cloud.annotation.ApiVersion;
import com.snailscoder.core.cloud.annotation.UrlVersion;
import com.snailscoder.core.cloud.version.SnailMediaType;
...
/**
 * 支持 snail-boot 的 版本 处理
 *
 * @see UrlVersion
 * @see ApiVersion
 * @author L.cm
 */
public class SnailSpringMvcContract extends SpringMvcContract {

	public SnailSpringMvcContract(List<AnnotatedParameterProcessor> annotatedParameterProcessors, ConversionService conversionService) {
	...
	protected void processAnnotationOnMethod(MethodMetadata data, Annotation methodAnnotation, Method method) {
		if (RequestMapping.class.isInstance(methodAnnotation) || methodAnnotation.annotationType().isAnnotationPresent(RequestMapping.class)) {
			...
			if (apiVersion != null && StringUtil.isNotBlank(apiVersion.value())) {
				SnailMediaType SnailMediaType = new SnailMediaType(apiVersion.value());
				data.template().header(HttpHeaders.ACCEPT, SnailMediaType.toString());
			}
		}
	}
