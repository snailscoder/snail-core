 ...
 * <p>
 *     url: /v1/user/{id}
 *     header: Accept application/vnd.snail.VERSION+json
 * </p>
 *
 * 注意：c 代表客户端版本
 *
 * @author L.cm
 */
public class SnailRequestMappingHandlerMapping extends RequestMappingHandlerMapping {
	...
	private RequestMappingInfo getApiVersionMappingInfo(Method method, Class<?> handlerType) {
		...
		// 如果url版本不存在
		if (nonUrlVersion) {
			String vsersionMediaTypes = new SnailMediaType(apiVersion.value()).toString();
